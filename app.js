...
function syncLikeButtonsForPost(postId, liked, count, sourceBtn = null) {
  ...
  document
    .querySelectorAll(`[data-like-post-id="${CSS.escape(key)}"]`)
    .forEach((btnEl) => {
      const icon = btnEl.querySelector("i");
      const countEl = btnEl.querySelector(".like-count");
      const isReelButton =
        btnEl.classList.contains("reel-action-btn") ||
        !!btnEl.closest(".reel-actions");
...
function getCommentSections(postId) {
  return Array.from(
    document.querySelectorAll(exactIdSelector(`comments-${idKey(postId)}`)),
  );
}
...
function getPreferredCommentSection(postId, sourceEl = null) {
  ...
  if (sourceEl instanceof Element) {
    const direct = sections.find(
      (section) => section === sourceEl || section.contains(sourceEl),
    );
  ...
  return (
    sections.find((section) => section.classList.contains("comments-open")) ||
...
function getCommentListEl(postId, sourceEl = null) {
  ...
  return (
    section?.querySelector(exactIdSelector(`comment-list-${idKey(postId)}`)) ||
    null
  );
}
...
function getCommentSendBtn(postId, sourceEl = null) {
  const section = getPreferredCommentSection(postId, sourceEl);
  return (
    section?.querySelector(exactIdSelector(`comment-send-${idKey(postId)}`)) ||
    null
  );
}
...
function getCommentCancelBtn(postId, sourceEl = null) {
  const section = getPreferredCommentSection(postId, sourceEl);
  return (
    section?.querySelector(exactIdSelector(`cancel-reply-${idKey(postId)}`)) ||
    null
  );
}
...
window.postComment = async function (postId, inputEl, parentCommentId = null) {
  ...
  const previousCount = Number(
    commentCountCache[key] ??
      document.querySelector(`.comment-count-${CSS.escape(key)}`)
        ?.textContent ??
      0,
  );
...
window.toggleComments = async function (postId, triggerEl = null) {
  ...
  const list = commentSection?.querySelector(
    exactIdSelector(`comment-list-${key}`),
  );
  ...
  document.querySelectorAll(".reel-comments.comments-open").forEach((el) => {
    ...
    if (idKey(otherPostId) !== key)
      window._closeCommentSheet(otherPostId, true);
  });
...
window.handlePostSubmission = async function () {
  ...
  const originalPriceRaw = flashSaleEnabled
    ? document
        .getElementById("postOriginalPrice")
        ?.value.trim()
        .replace(/,/g, "") || ""
    : "";
