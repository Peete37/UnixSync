...
const savedSearchAlerts = JSON.parse(localStorage.getItem(SAVED_ALERTS_KEY) || '[]');
...
window.switchProfileTab = function (tabType, selectedBtn = null) {
    ...
    if (settingsBtn) {
        settingsBtn.classList.remove('bg-amber-400', 'text-black', 'border-amber-400');
        settingsBtn.classList.add('bg-slate-900', 'text-slate-300', 'border-slate-700');
    }
    ...
    if (selectedBtn) {
        ...
        settingsBtn.classList.remove('bg-slate-900', 'text-slate-300', 'border-slate-700');
    ...
    if (tabType === 'saved') {
        renderCartListView().catch(err => {
            ...
            if (wrapper) wrapper.innerHTML = `<p class="p-10 text-center text-slate-500 text-xs uppercase">Couldn't load your saved items. Please try again.</p>`;
        });
    }
};
...
function buildCartListMarkup() {
    ...
    return validItems.map(item => {
        ...
        if (firstUrl.startsWith('[')) {
            try { firstUrl = JSON.parse(firstUrl)[0] || ''; } catch (_) { /* leave as-is */ }
        }
...
async function loadFollowingFeed() {
    ...
    try {
        ...
        feed.innerHTML = posts.map(d => renderFeedCard(d.id, d)).join('') + `
...
async function loadProfileStats() {
    ...
    try {
        const [followersRes, followingRes, postsRes, bioRes] = await Promise.all([
            ...
            // cosmetic.
            supabase.from("posts").select("id, title, media_url, media_type, price").eq("user_id", currentUserData.id).eq("is_archived", false).order("created_at", { ascending: false }),
...
async function loadArchivedPostsIntoSheet() {
    ...
    bodyEl.innerHTML = posts.map(d => {
        ...
        if (d.media_url) {
            try {
                const urls = d.media_url.startsWith('[') ? JSON.parse(d.media_url) : [d.media_url];
                thumb = urls[0] || '';
            } catch (_) { thumb = d.media_url; }
        }
...
window.openPublicProfile = async function (userId) {
    ...
    try {
        const [followersRes, followingRes, postsRes, isFollowingRes, profileRowRes] = await Promise.all([
            ...
            supabase.from('posts').select('id, title, media_url, media_type, price').eq('user_id', userId).eq('is_archived', false).order('created_at', { ascending: false }),
            currentUserData
