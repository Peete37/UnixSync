...
function pickThumbnailUrl(post) {
    const parseFirst = (raw) => {
        if (!raw) return '';
        if (raw.startsWith('[')) {
            try { return JSON.parse(raw)[0] || ''; } catch (_) { return raw; }
        }
        return raw;
    };
...
function getDeviceSessionId() {
    ...
    if (!localStorage.getItem('campus_market_session_started_at')) {
        localStorage.setItem('campus_market_session_started_at', String(Date.now()));
    }
...
async function registerDeviceSession() {
    ...
    try {
        ...
        if (!activeSessions || activeSessions.length <= MAX_CONCURRENT_SESSIONS) return;
...
window.signOutOtherSessions = async function () {
    if (!currentUserData?.id) { showToast('Sign in first.'); return; }
...
function startSessionExpiryWatch() {
    ...
    sessionIdleCheckTimer = setInterval(() => {
        if (!currentUserData?.id) return;
        const now         = Date.now();
        const startedAt    = Number(localStorage.getItem('campus_market_session_started_at') || now);
        const lastActivity = Number(localStorage.getItem('campus_market_last_activity_at') || now);
...
function stopSessionExpiryWatch() {
    if (sessionIdleCheckTimer) { clearInterval(sessionIdleCheckTimer); sessionIdleCheckTimer = null; }
}
    ...
    sessionRevokeChannel = supabase
        ...
        .on('postgres_changes', {
            ...
            if (row?.session_id === getDeviceSessionId() && row.revoked) {
                showToast("You've been signed out — this session was ended from another device.");
                window.logout();
            }
        })
        .subscribe();
...
function stopSessionRevokeListener() {
    if (sessionRevokeChannel) { supabase.removeChannel(sessionRevokeChannel); sessionRevokeChannel = null; }
}
...
async function checkSystemAnnouncements() {
    try {
        ...
        // last dismissed.
        if (localStorage.getItem('campus_market_dismissed_announcement') === announcement.id) {
            banner.classList.add('hidden');
            return;
        }
        ...
        document.getElementById('system-announcement-title').textContent = announcement.title;
        document.getElementById('system-announcement-message').textContent = announcement.message;
...
window.dismissSystemAnnouncement = function () {
    ...
    if (banner?.dataset.announcementId) {
        localStorage.setItem('campus_market_dismissed_announcement', banner.dataset.announcementId);
    }
    banner?.classList.add('hidden');
};
...
function startSystemAnnouncementWatch() {
    ...
    systemAnnouncementTimer = setInterval(checkSystemAnnouncements, SYSTEM_ANNOUNCEMENT_CHECK_MS);
}
...
const conversationLastRead = safeStorageJsonParse('campus_market_dm_last_read', {});
...
function updateDmUnreadBadge() {
    ...
    [document.getElementById('dms-unread-badge'), document.getElementById('dms-unread-badge-desktop')].forEach(badge => {
        if (!badge) return;
        if (unreadCount > 0) {
...
let _lastScrollAt = 0;
window.addEventListener('scroll', () => { _lastScrollAt = Date.now(); }, { passive: true, capture: true });
function _isActivelyScrolling() {
    return Date.now() - _lastScrollAt < 150;
}
...
window.addEventListener('popstate', () => {
    ...
    if (_uiStack.length > 0) {
        ...
        try { history.pushState({ uiView: 'keep' }, ''); } catch (_) {}
        return;
    }
    ...
    if (_viewHistory.length > 1) {
        ...
        if (prev) {
            ...
            try { window.navigateTo(prev); }
            finally { window._isViewNavInProgress = false; }
        }
        try { history.pushState({ uiView: _viewHistory[_viewHistory.length - 1] || 'base' }, ''); } catch (_) {}
        return;
    }
    ...
    if (!_exitConfirmArmed) {
        ...
        try { history.pushState({ uiExitGuard: true }, ''); } catch (_) {}
        clearTimeout(_exitConfirmTimer);
        _exitConfirmTimer = setTimeout(() => { _exitConfirmArmed = false; }, 2000);
        return;
    }
...
window._formatPriceInput = function (el) {
    ...
    intPart = (intPart || '').replace(/^0+(?=\d)/, ''); // "007" -> "7", but a lone "0" stays
...
function getFeedScore(post) {
    ...
    const engagement = (parseInt(post?.likes_count || 0, 10) * 1.35) + (parseInt(post?.comments_count || 0, 10) * 2.1);
...
async function fetchSuggestedReelsPool() {
    ...
    if (Date.now() - _suggestedReelsFetchedAt < 5 * 60 * 1000 && suggestedReelsPool.length > 0) {
...
function interleaveSuggestedReelsIncremental(regularCardsHtml, pool, alreadyShownIds) {
    if (!pool || pool.length === 0) return regularCardsHtml;
    ...
    for (const cardHtml of regularCardsHtml) {
        ...
        if (_reelInterleaveSinceLastInsert >= _reelInterleaveNextGap && _reelInterleavePoolIndex < available.length) {
...
function refreshReelsCache() {
    allReelsCache = allCachedPosts.filter(({ data: d }) => d?.media_type === 'video');
...
function applyFeedRankingToCache() {
    ...
    allCachedPosts.sort((a, b) => {
        ...
        return Number((b?.data?.id ?? b?.id) || 0) - Number((a?.data?.id ?? a?.id) || 0);
    });
...
function getSafeSwapZoneSuggestions(post) {
    ...
    if (Array.isArray(byInstitution) && byInstitution.length > 0) return byInstitution;
...
function renderSimilarListingsBlock(post) {
    ...
    const candidates = allCachedPosts
        ...
        .filter(d => d && idKey(d.id) !== idKey(post.id) && (d.type || 'product') === targetType);
    ...
    scored.sort((a, b) => b.score - a.score || (b.createdAt > a.createdAt ? 1 : -1));
...
function persistAlertedPostIds() {
    localStorage.setItem(ALERT_NOTIFIED_POSTS_KEY, JSON.stringify([...alertedPostIds]));
}
...
function postMatchesSavedAlert(post, alert) {
    ...
    const haystack = [post.title, post.description, post.user_name, post.institution, post.region, post.type]
        .map(v => String(v || '').toLowerCase())
        .join(' ');
    if (!haystack.includes(alert.term)) return false;
    if (alert.postType && alert.postType !== 'all' && (post.type || 'product') !== alert.postType) return false;
    if (alert.scope === 'institution' && currentUserData?.institution && post.institution !== currentUserData.institution) return false;
    if (alert.scope === 'region' && currentUserData?.region && post.region !== currentUserData.region) return false;
    return true;
}

function notifySavedAlertsForPosts(posts, { source = 'feed' } = {}) {
    if (!Array.isArray(posts) || posts.length === 0 || savedSearchAlerts.length === 0) return;
    const matched = [];
    posts.forEach(post => {
        const key = idKey(post?.id);
        if (!key || alertedPostIds.has(key)) return;
        if (post?.user_id && currentUserData?.id && post.user_id === currentUserData.id) return;
        const hit = savedSearchAlerts.find(alert => postMatchesSavedAlert(post, alert));
        if (!hit) return;
    ...
    const label = preview.alert.term.length > 28 ? `${preview.alert.term.slice(0, 28)}…` : preview.alert.term;
...
window.saveSearchAlert = function (term) {
    ...
    if (!normalized) { showToast('Type a keyword before saving an alert.'); return; }
    if (savedSearchAlerts.some(alert => alert.term === normalized)) { showToast('That alert is already saved.'); return; }
...
window.openRateSellerSheet = async function (sellerId, sellerName) {
    if (!currentUserData) { showToast('Please sign in to leave a rating.'); return; }
    if (sellerId === currentUserData.id) { showToast("You can't rate yourself."); return; }
    ...
    pushUiState('manage-listing-modal', () => window.closeManageListingSheet(true));
...
window._submitSellerRating = async function (sellerId) {
    ...
    if (stars < 1 || stars > 5) { showToast('Please select a star rating.'); return; }
    ...
    const { error } = await supabase
        .from('seller_ratings')
        .upsert(
            { seller_id: sellerId, rater_id: currentUserData.id, stars, comment: comment || null },
...
window.saveOnboarding = async function () {
    ...
    if (!region)      { showToast('Please select your region.'); return; }
    if (!institution) { showToast('Please select your institution.'); return; }
...
window.confirmSignOut = function () {
    showConfirmDialog({
        title: 'Sign out?',
        message: "You'll need to sign in again to post, message, or manage your account.",
...
function buildFeedQuery(baseFilter, cursor = null, limit = FEED_PAGE_SIZE) {
    let q = supabase.from("posts").select(FEED_SELECT_COLUMNS).eq("is_archived", false);
...
async function subscribeFeed(baseFilter = null) {
    ...
    try {
        const data = await fetchFeedSnapshot(() => buildFeedQuery(baseFilter, null, FEED_PAGE_SIZE));
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "posts" }, (payload) => {
            ...
            _feedRefreshDebounceTimer = setTimeout(async () => {
                try {
                    ...
                    const data = await fetchFeedSnapshot(() => buildFeedQuery(baseFilter, null, currentCount));
...
async function loadNextFeedPage() {
    ...
    try {
        const data = await fetchFeedSnapshot(() => buildFeedQuery(currentFeedBaseFilter, feedCursor, FEED_PAGE_SIZE));
...
function getNavLabelEl(btn) {
    if (!btn) return null;
    return [...btn.querySelectorAll('span')].find(el => !el.classList.contains('nav-cart-badge') && !el.classList.contains('badge-dot')) || null;
}
...
window.openDetail = async function (postId, fromBack = false) {
    ...
    try {
        const { data: d, error } = await supabase.from("posts").select(FEED_SELECT_COLUMNS).eq("id", postId).single();
        ...
        const hasDiscountDetail = saleActiveDetail && d.original_price != null && Number(d.original_price) > 0 && Number(d.original_price) !== Number(d.price || 0);
        ...
        const heartClassDetail = isLikedDetail ? 'fas fa-heart text-rose-500' : 'far fa-heart text-slate-300';
        const displayLikesDetail = parseInt(d.likes_count || 0);
        const displayCommentsDetail = commentCountCache[d.id] ?? parseInt(d.comments_count || 0);
        ...
        content.innerHTML = `
            ...
            <div class="p-6 space-y-4 bg-[#0f172a] rounded-t-3xl relative shadow-[0_-12px_24px_-8px_rgba(0,0,0,0.5)]">
                ...
                <div class="flex items-center gap-4 pt-1 pb-1 border-y border-slate-800/60">
                    <button onclick="likePost('${escAttr(d.id)}', this)" data-like-post-id="${escAttr(idKey(d.id))}" data-liked="${isLikedDetail ? 'true' : 'false'}" class="flex items-center gap-1.5 active:scale-90 transition ${isLikedDetail ? 'text-rose-500' : 'text-slate-300'}">
                        <i class="${heartClassDetail} text-2xl"></i>
                        <span class="like-count text-sm font-semibold text-slate-300">${displayLikesDetail}</span>
                    </button>
                    <button onclick="toggleComments('${escAttr(d.id)}', this)" class="flex items-center gap-1.5 text-slate-300 hover:text-amber-400 transition active:scale-90">
            ...
            <div id="comments-${escAttr(d.id)}" class="hidden reel-comments">
                ...
                <div class="comments-input-row flex items-center gap-1.5">
                    ...
                    <button
                        ...
                        disabled
                        onclick="window._submitFromSendBtn('${escAttr(d.id)}', this)"
...
window.openManageListingSheet = async function (postId) {
    ...
    const saleEndsValue = post.sale_ends_at ? new Date(post.sale_ends_at).toISOString().slice(0, 16) : '';
...
window._saveManageListing = async function (postId) {
    ...
    if (originalPriceRaw && (isNaN(parsedOriginalPrice) || parsedOriginalPrice < 0)) {
...
window.signInWithEmailPassword = async function (email, password) {
    ...
    try {
        ...
        try {
            const { data: lockoutCheck } = await supabase.functions.invoke('login-guard', {
                body: { action: 'check', email }
            });
            if (lockoutCheck?.locked) {
                const mins = Math.ceil((lockoutCheck.retryAfterSeconds || 60) / 60);
                showToast(`Too many failed attempts. Try again in ${mins} minute${mins === 1 ? '' : 's'}.`);
                return;
            }
        } catch (_) { /* fail open — see comment above */ }
        ...
        if (error) {
            ...
            supabase.functions.invoke('login-guard', { body: { action: 'failure', email } }).catch(() => {});
            throw error;
        }
        ...
        supabase.functions.invoke('login-guard', { body: { action: 'success', email } }).catch(() => {});
...
window.registerWithEmail = async function (name, email, password) {
    ...
    if (!_turnstileToken) {
        showToast('Please complete the verification check above before continuing.');
        return;
    }
    ...
    try {
        ...
        const { data, error } = await supabase.auth.signUp({
            email,
            password,
            options: { data: { full_name: name }, captchaToken: _turnstileToken }
        });
        if (error) throw error;
        startNewDeviceSession();
...
window.handleAvatarUpload = async function (inputEl) {
    ...
    try {
        ...
        if (previewEl) {
            ...
            const fallbackAvatar = currentUserData.user_metadata?.avatar_url
...
function syncLikeButtonsForPost(postId, liked, count, sourceBtn = null) {
    ...
    document.querySelectorAll(`[data-like-post-id="${CSS.escape(key)}"]`).forEach(btnEl => {
        const icon = btnEl.querySelector('i');
        const countEl = btnEl.querySelector('.like-count');
        const isReelButton = btnEl.classList.contains('reel-action-btn') || !!btnEl.closest('.reel-actions');

        btnEl.dataset.liked = liked ? 'true' : 'false';
        btnEl.classList.toggle('text-rose-500', liked);
        btnEl.classList.toggle('text-slate-300', !liked && !isReelButton);

        if (icon) {
            icon.classList.remove('fas', 'far', 'text-rose-500', 'text-slate-300', 'text-white');
            icon.classList.add(liked ? 'fas' : 'far', 'fa-heart');
            icon.classList.add(liked ? 'text-rose-500' : (isReelButton ? 'text-white' : 'text-slate-300'));

            if (liked && sourceBtn && btnEl === sourceBtn) {
                icon.classList.add('like-pop-anim');
                icon.addEventListener('animationend', () => {
                    icon.classList.remove('like-pop-anim');
                }, { once: true });
            }
        }
...
window.likePost = async function (postId, btn) {
    ...
    if (liked) {
        ...
        syncLikeButtonsForPost(key, false, currentCount);
    } else {
    ...
    try {
        ...
        if (liked) {
            likedPostIds.add(key);
            currentCount = currentCount + 1;
            syncLikeButtonsForPost(key, true, currentCount);
        } else {
            likedPostIds.delete(key);
            currentCount = Math.max(0, currentCount - 1);
            syncLikeButtonsForPost(key, false, currentCount);
...
function getCommentSections(postId) {
    return Array.from(document.querySelectorAll(exactIdSelector(`comments-${idKey(postId)}`)));
}

function getPreferredCommentSection(postId, sourceEl = null) {
    const sections = getCommentSections(postId);
    if (sections.length === 0) return null;

    if (sourceEl instanceof Element) {
        const direct = sections.find(section => section === sourceEl || section.contains(sourceEl));
        if (direct) return direct;
    }

    const active = document.activeElement instanceof Element ? document.activeElement : null;
    if (active) {
        const owning = sections.find(section => section.contains(active));
        if (owning) return owning;
    }

    return sections.find(section => section.classList.contains('comments-open'))
        || sections.find(section => !section.classList.contains('hidden'))
        || sections[sections.length - 1];
}

function getCommentListEl(postId, sourceEl = null) {
    const section = getPreferredCommentSection(postId, sourceEl);
    return section?.querySelector(exactIdSelector(`comment-list-${idKey(postId)}`)) || null;
}

function getCommentInputEl(postId, sourceEl = null) {
    const section = getPreferredCommentSection(postId, sourceEl);
    return section?.querySelector('.comment-input-field') || null;
}

function getCommentSendBtn(postId, sourceEl = null) {
    const section = getPreferredCommentSection(postId, sourceEl);
    return section?.querySelector(exactIdSelector(`comment-send-${idKey(postId)}`)) || null;
}

function getCommentCancelBtn(postId, sourceEl = null) {
    const section = getPreferredCommentSection(postId, sourceEl);
    return section?.querySelector(exactIdSelector(`cancel-reply-${idKey(postId)}`)) || null;
}
...
window.postComment = async function(postId, inputEl, parentCommentId = null) {
    ...
    const previousCount = Number(commentCountCache[key] ?? document.querySelector(`.comment-count-${CSS.escape(key)}`)?.textContent ?? 0);
    ...
    updateCommentCountUI(key, previousCount + 1);
    if (idKey(_currentDetailPostId) === key) {
        loadCommentPreview(key).catch(() => {});
    }
    if (list) {
        const metadata = currentUserData.user_metadata || {};
    ...
    try {
        ...
        updateCommentCountUI(key, previousCount);
        if (idKey(_currentDetailPostId) === key) {
            loadCommentPreview(key).catch(() => {});
        }
    }
...
window.startCommentReply = function (postId, commentId, commentAuthor, sourceEl = null) {
    const key = idKey(postId);
    activeReplyTarget[key] = commentId;
    const input = getCommentInputEl(key, sourceEl);
    if (input) {
        input.placeholder = `Replying to ${commentAuthor}…`;
        input.dataset.replyTo = commentId;
        input.focus();
    }
    const cancelBtn = getCommentCancelBtn(key, sourceEl);
    if (cancelBtn) cancelBtn.classList.remove('hidden');
};

window.cancelCommentReply = function (postId, sourceEl = null) {
    const key = idKey(postId);
    delete activeReplyTarget[key];
    const input = getCommentInputEl(key, sourceEl);
    if (input) {
...
window.submitCommentFromInput = function (postId, inputEl) {
    ...
    window.postComment(key, inputEl, parentId);
    ...
    showConfirmDialog({
        ...
        onConfirm: async () => {
            try {
                ...
                const key = idKey(postId);
                const newCount = Math.max(0, (commentCountCache[key] ?? 1) - 1);
                updateCommentCountUI(key, newCount);
                if (idKey(_currentDetailPostId) === key) {
                    loadCommentPreview(key).catch(() => {});
                }
            } catch (err) {
...
async function loadCommentPreview(postId) {
    ...
    const previewHtml = comments.map(c => renderCommentItem(c, postId, { preview: true })).join('');
    const viewAllLabel = (count && count > comments.length)
        ? `View all ${count} comments`
        : 'View comments';

    container.innerHTML = `
        ${previewHtml}
        <button onclick="toggleComments('${escAttr(postId)}', this)" class="text-[11px] text-amber-400 hover:text-amber-300 transition font-bold uppercase tracking-widest">
            ${esc(viewAllLabel)}
        </button>`;
}
...
window.toggleComments = async function (postId, triggerEl = null) {
    ...
    const list           = commentSection?.querySelector(exactIdSelector(`comment-list-${key}`));
    ...
    document.querySelectorAll('.reel-comments.comments-open').forEach(el => {
        const otherPostId = el.id.replace('comments-', '');
        if (idKey(otherPostId) !== key) window._closeCommentSheet(otherPostId, true);
    });
    ...
    const chanId = `comments-live-${key}`;
...
try {
    const zoomObserver = new MutationObserver((mutations) => {
        for (const m of mutations) {
            ...
            for (const n of m.addedNodes) {
                ...
                if (n.matches?.('img, video, picture')) { scheduleZoomScopeRescan(); break; }
                if (n.querySelector?.('img, video, picture')) { scheduleZoomScopeRescan(); break; }
            }
    ...
    zoomObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['src', 'class'] });
} catch (_) { /* MutationObserver isn't supported (very old browser) — fall back silently */ }
...
function showConfirmDialog({ title, message, confirmLabel = 'Delete', danger = true, requireText = null, onConfirm }) {
...
async function syncFollowingIds() {
    ...
    try {
        ...
        (data || []).forEach(row => followingUserIds.add(idKey(row.following_id)));
    } catch (err) {
...
window.submitVerificationRequest = async function (eduEmail) {
    ...
    const looksLikeEduEmail = /^[^\s@]+@[^\s@]+\.(edu(\.[a-z]{2})?|ac\.[a-z]{2})$/i.test(email);
    ...
    try {
        ...
        showToast("Verification request submitted — we'll email you a confirmation link.");
        ...
        showToast("Couldn't submit right now. This feature needs a database update — try again later.");
    }
};
...
async function applyVerifiedBadgeWhenReady(userId, targetSelector) {
    ...
    document.querySelectorAll(targetSelector).forEach(el => {
        if (!el.querySelector('.verified-badge-inline')) {
            el.insertAdjacentHTML('beforeend', `<span class="verified-badge-inline">${verifiedBadgeHtml()}</span>`);
        }
    });
}
...
window.submitSellerRating = async function (sellerId, stars, comment = '') {
    ...
    try {
        ...
        if (document.getElementById('public-profile-overlay')?.classList.contains('sheet-open')) {
...
async function fetchSellerRatingSummary(sellerId) {
    ...
    try {
        ...
        const average = count > 0 ? data.reduce((sum, r) => sum + r.stars, 0) / count : 0;
...
window.openRatingPrompt = function (sellerId) {
    openOptionsMenu([
        { label: '★☆☆☆☆ — 1 star', icon: 'far fa-star', action: () => window.submitSellerRating(sellerId, 1) },
        { label: '★★☆☆☆ — 2 stars', icon: 'far fa-star', action: () => window.submitSellerRating(sellerId, 2) },
        { label: '★★★☆☆ — 3 stars', icon: 'far fa-star', action: () => window.submitSellerRating(sellerId, 3) },
        { label: '★★★★☆ — 4 stars', icon: 'far fa-star', action: () => window.submitSellerRating(sellerId, 4) },
        { label: '★★★★★ — 5 stars', icon: 'far fa-star', action: () => window.submitSellerRating(sellerId, 5) }
    ]);
};
    ...
    try {
        ...
        if (targetType === 'post') {
            ...
            await supabase.from('posts')
                .update({ is_archived: true, archived_at: new Date().toISOString(), status: 'archived_auto_reported' })
                .eq('id', targetId);
            try { renderFeedFromCache(); } catch (_) {}
            ...
            await supabase.from('comments')
                ...
                .eq('id', targetId);
            document.querySelectorAll(`[id="comment-item-${CSS.escape(targetId)}"]`).forEach(el => el.remove());
        }
    } catch (err) {
        ...
        console.warn('Auto-moderation check failed (likely missing status column):', err);
    }
}
...
function openReportReasonMenu(targetType, targetId, authorId = null, authorName = 'this student') {
    openOptionsMenu([
        { label: 'Spam / scam', icon: 'fas fa-shield-halved', action: () => submitReport(targetType, targetId, 'spam_or_scam') },
        { label: 'Misleading description', icon: 'fas fa-circle-exclamation', action: () => submitReport(targetType, targetId, 'misleading_description') },
        { label: 'Unsafe meetup request', icon: 'fas fa-location-dot', action: () => submitReport(targetType, targetId, 'unsafe_meetup_request') },
        { label: 'Harassment / abuse', icon: 'fas fa-user-slash', action: () => submitReport(targetType, targetId, 'harassment_or_abuse') },
        { divider: true },
        { label: `Block ${authorName}`, icon: 'fas fa-user-slash', danger: true, action: () => window.blockUser(authorId, authorName) }
    ]);
}

window.openPostOptionsMenu = function (postId, isOwn, authorId = null, authorName = 'this student', isSuggested = false) {
    const sharedItems = [
        { label: 'View Profile', icon: 'fas fa-user', action: () => window.openPublicProfile(authorId) },
        { label: 'Copy Link', icon: 'fas fa-link', action: () => window.copyPostLink(postId) },
        { label: "Why you're seeing this post", icon: 'fas fa-circle-info', action: () => window.openWhySeeingPost(authorId, isSuggested) },
    ];
    ...
    const items = isOwn
        ? [
            ...sharedItems,
            { divider: true },
            { label: 'Manage listing', icon: 'fas fa-sliders', action: () => window.openManageListingSheet(postId) },
            { label: 'Archive or delete listing', icon: 'fas fa-box-archive', danger: true, action: () => window.deletePost(postId) }
          ]
        : [
            ...sharedItems,
            { divider: true },
            { label: 'Report listing', icon: 'fas fa-flag', action: () => openReportReasonMenu('post', postId, authorId, authorName) },
            { divider: true },
            { label: `Block ${authorName}`, icon: 'fas fa-user-slash', danger: true, action: () => window.blockUser(authorId, authorName) }
          ];
...
window.openCommentOptionsMenu = function (commentId, postId, isOwn, authorId = null, authorName = 'this student') {
    const items = isOwn
        ? [
            { label: 'Delete comment', icon: 'fas fa-xmark', danger: true, action: () => window.deleteComment(commentId, postId) }
          ]
        : [
            { label: 'Report comment', icon: 'fas fa-flag', action: () => openReportReasonMenu('comment', commentId, authorId, authorName) },
            { divider: true },
            { label: `Block ${authorName}`, icon: 'fas fa-user-slash', danger: true, action: () => window.blockUser(authorId, authorName) }
          ];
    openOptionsMenu(items);
    ...
    <div class="bg-slate-900 border-b border-slate-800/60 w-full" id="feed-card-${escAttr(id)}">
        ...
        <div class="px-3 pt-2.5 pb-1 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <button onclick="likePost('${escAttr(id)}', this)" data-like-post-id="${escAttr(idKey(id))}" data-liked="${likedData}" class="flex items-center gap-1 active:scale-90 transition ${isLiked ? 'text-rose-500' : ''}">
                    <i class="${heartClass} text-xl"></i>
                    <span class="like-count text-xs font-semibold text-slate-300">${displayLikes}</span>
                </button>
                <button onclick="toggleComments('${escAttr(id)}', this)" class="flex items-center gap-1 text-slate-300 hover:text-amber-400 transition active:scale-90">
        ...
        <div id="comments-${escAttr(id)}" class="hidden px-3 pb-3 space-y-2 border-t border-slate-800/60 pt-2">
            <div class="flex items-center gap-1.5">
                ...
                <button
                    ...
                    disabled
                    onclick="window._submitFromSendBtn('${escAttr(id)}', this)"
                    class="comment-send-btn shrink-0 w-8 h-8 rounded-xl bg-amber-400 text-black flex items-center justify-center transition disabled:opacity-30 disabled:cursor-not-allowed active:scale-90"
                    aria-label="Send comment"
                >
                    <i class="fas fa-paper-plane text-[11px]"></i>
...
function renderFeedMasonryCard(id, d, options = {}) {
    ...
    const hasDiscount = saleActive && d.original_price != null && Number(d.original_price) > 0 && Number(d.original_price) !== Number(d.price || 0);
...
function renderServiceGrid() {
    ...
    feed.innerHTML = `<div class="masonry-columns-services py-2">${
        services.map(({ id, data: d }) => renderServiceGridCard(id, d)).join('')
    }</div>`;

    const canWidenServices = currentCampusScope !== 'everywhere' && currentUserData?.institution;
    feed.innerHTML += `
        <div id="feed-load-more-sentinel" class="py-6 text-center space-y-2 px-6">
            ${feedHasMore
                ? ''
...
window.openServiceReelViewer = async function (startPostId) {
    ...
    const services = allCachedPosts.filter(({ data: d }) => d.type === 'skill').map(({ data: d }) => d);
    ...
    feed.innerHTML = services.map(d => renderReelCard(idKey(d.id), d, true)).join('');

    requestAnimationFrame(() => {
        const targetCard = document.getElementById(`reel-card-${CSS.escape(idKey(startPostId))}`);
        if (targetCard) {
            targetCard.scrollIntoView({ block: 'start' });
            const targetMedia = targetCard.querySelector('.reel-video');
            if (targetMedia && targetMedia.dataset.src) targetMedia.src = targetMedia.dataset.src;
        }
    });
...
function pauseAllReelVideos() {
    document.querySelectorAll('.reel-video, .feed-lazy-video').forEach(video => {
        ...
        try {
            ...
            if ((video.classList.contains('feed-lazy-video') || video.dataset.src) && video.src) {
                video.removeAttribute('src');
                video.load();
...
function setupFeedCommentAutoClose() {
    ...
    feedCommentAutoCloseObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            const commentSection = getPreferredCommentSection(postId);
            if (
                commentSection
            ...
            if (entry.isIntersecting && entry.intersectionRatio >= 0.6) {
                ...
                if (video) {
                    document.querySelectorAll('.reel-video').forEach(v => {
                        if (v !== video && v.tagName === 'VIDEO') { v.pause(); v.muted = true; v.currentTime = v.currentTime; }
                    });
...
function renderReelCard(id, d, lazy = false) {
    ...
    const srcAttr = lazy ? `data-src="${esc(primaryUrl)}"` : `src="${esc(primaryUrl)}"`;
    const mediaBlock = isVideo
    ...
    <div class="reel-card" id="reel-card-${escAttr(id)}">
        ...
        <div class="reel-actions">
            <button onclick="likePost('${escAttr(id)}', this)" data-like-post-id="${escAttr(idKey(id))}" data-liked="${isLiked ? 'true' : 'false'}" class="reel-action-btn flex flex-col items-center">
                <i class="${heartClass} text-2xl"></i>
                <span class="like-count text-white text-[10px] font-bold mt-1">${displayLikes}</span>
            </button>
            <button onclick="toggleComments('${escAttr(id)}', this)" class="reel-action-btn flex flex-col items-center">
...
function renderReelsFeed() {
    ...
    // always consistent.
    const reels = (allReelsCache && allReelsCache.length && allReelsCache.every(p => p?.data?.media_type === 'video'))
        ? allReelsCache
    ...
    feed.innerHTML = reels.map(({ id, data: d }) => renderReelCard(id, d, true)).join('');
...
async function hydrateCartItemsFromSource() {
    ...
    if (missingIds.length) {
        try {
            const { data, error } = await supabase
                .from('posts')
                .select('id, title, price, media_url, media_type, institution, type, user_id, user_name, user_avatar')
                .in('id', missingIds);
            if (error) throw error;
...
function getValidCartItems() {
    ...
    if (validItems.length !== userCartList.length) {
        ...
        try { localStorage.setItem('campus_market_cart', JSON.stringify(userCartList)); } catch (_) {}
    }
    return validItems;
}
...
function buildCartListMarkup() {
    ...
    const cards = validItems.map(item => {
        ...
        const canContact = !!item.user_id && (!currentUserData || idKey(item.user_id) !== idKey(currentUserData.id));
        const typeLabel = item.type === 'skill' ? 'Service' : 'Product';
        const institution = item.institution ? esc(item.institution) : 'Campus listing';
...
window.toggleFollow = async function (targetUserId, targetName, targetAvatar) {
    // Fix: native alert() replaced with showToast for consistency.
    if (!currentUserData) { showToast("Please sign in to follow."); return; }
...
function updateFollowButtons(targetUserId, isFollowing) {
    const key = idKey(targetUserId);
    if (isFollowing) followingUserIds.add(key); else followingUserIds.delete(key);
...
window.restorePost = async function (postId) {
    ...
    try {
        ...
        if (wasReportTriggered) {
            supabase.from('restore_audit_log').insert({
                post_id: String(postId),
                restored_by: currentUserData.id,
                was_report_triggered: true,
                restored_at: new Date().toISOString()
            }).then(({ error: logErr }) => {
                if (logErr) console.warn('Restore audit log failed (non-fatal):', logErr);
            });
        }
...
async function fetchArchivedPosts() {
    ...
    try {
        const { data, error } = await supabase
            .from("posts")
            .select("id, title, media_url, thumbnail_url, media_type, price, archived_at")
            .eq("user_id", currentUserData.id)
            .eq("is_archived", true)
...
async function _deletePostById(postId) {
    ...
    if (currentPost?.media_url) {
        ...
        const storagePaths = targets
            ...
            .filter(Boolean);
        if (storagePaths.length) await supabase.storage.from("posts").remove(storagePaths);
    }
...
window.deletePost = function (postId) {
    ...
    showConfirmDialog({
        ...
        message: "Uni-Sync now tries a safer archive-first workflow. If your database doesn't support archiving yet, it falls back to permanent deletion.",
        confirmLabel: 'Archive',
        danger: true,
        onConfirm: async () => {
            try {
                const result = await _deletePostById(postId);
                showToast(result?.mode === 'archived' ? "Listing archived successfully! ✓" : "Post deleted successfully! ✓");
                renderFeedFromCache();
            } catch (err) {
...
async function loadFollowingFeed() {
    ...
    try {
        ...
        posts.forEach(d => { wireCarouselCounters(d.id); fetchAndCacheCommentCount(d.id); });
...
async function loadNextFollowingPage() {
    ...
    try {
        let followingQuery = supabase
            ...
            .order("created_at", { ascending: false })
            .order("id", { ascending: false });
        followingQuery = applyCursorToPostQuery(followingQuery, followingFeedCursor).limit(FEED_PAGE_SIZE);
...
function renderFeedFromCache() {
    ...
    if (currentFeedType === 'reels') {
        ...
        // of re-filtering.
        const videos = (allReelsCache && allReelsCache.length) ? allReelsCache
            : allCachedPosts.filter(({ data: d }) => d.media_type === 'video');
    ...
    const regularCardsHtml = allCachedPosts.map(({ id, data: d }) => cardRenderer(id, d));
    ...
    if (currentFeedType === 'all' && allCachedPosts.length > 0) {
        ...
        fetchSuggestedReelsPool().then(pool => {
            ...
            const regularCards = allCachedPosts.map(({ id, data: d }) => renderFeedMasonryCard(id, d));
            const merged = interleaveSuggestedReels(regularCards, pool, alreadyShownIds);
...
function reopenActiveCommentSheets() {
    openCommentIds.forEach(postId => {
        ...
        if (section) {
            ...
            const list = getCommentListEl(postId, section);
            if (list) {
                supabase.from('comments').select('*').eq('post_id', postId).order('created_at', { ascending: true })
                    .then(({ data: comments }) => {
                        ...
                        topLevel.forEach(c => {
                            ...
                            replies.filter(r => r.parent_comment_id && idKey(r.parent_comment_id) === idKey(c.id)).forEach(r => {
...
function appendFeedCards(newItems) {
    ...
    if (blockedUserIds.size > 0 && newItems.some(({ data: d }) => blockedUserIds.has(idKey(d.user_id)))) {
        renderFeedFromCache();
        return;
    }

    if (currentFeedType !== 'all' && currentFeedType !== 'product' && currentFeedType !== 'skill') {
        renderFeedFromCache();
        return;
    }
    ...
    const displayedIds = Array.from(
        document.querySelectorAll('#posts-feed [id^="feed-card-"], #posts-feed [id^="grid-card-"]')
    ...
    if (displayedIds.length > 0) {
        ...
        const stillInOrder = displayedIds.length <= previousOrderIds.length &&
    ...
    let container, newCardsHtml = '';

    if (isAllTab) {
        container = feed.querySelector('.masonry-columns-feed');
        if (!container) { renderFeedFromCache(); return; }
        const plainCardsHtml = newItems.map(({ id, data: d }) => renderFeedMasonryCard(id, d));
        ...
        const alreadyShownIds = new Set(allCachedPosts.map(({ id }) => idKey(id)));
        newCardsHtml = interleaveSuggestedReelsIncremental(plainCardsHtml, pool, alreadyShownIds).join('');
    } else if (currentFeedType === 'product') {
        container = feed.querySelector('.masonry-columns');
        if (!container) { renderFeedFromCache(); return; }
        const products = newItems.filter(({ data: d }) => (d.type || 'product') === 'product');
        if (products.length > 0) newCardsHtml = products.map(({ id, data: d }) => renderProductGridCard(id, d)).join('');
    } else {
        container = feed.querySelector('.masonry-columns-services');
        if (!container) { renderFeedFromCache(); return; }
        const services = newItems.filter(({ data: d }) => d.type === 'skill');
        if (services.length > 0) newCardsHtml = services.map(({ id, data: d }) => renderServiceGridCard(id, d)).join('');
    }
    ...
    if (sentinel) {
        ...
        sentinel.innerHTML = feedHasMore
            ? ''
            : canWiden
                ? buildScopeWidenPrompt({ contextLabel: currentFeedType === 'product' ? 'products' : currentFeedType === 'skill' ? 'services' : 'posts' })
                : `<p class="text-center text-slate-600 text-[10px] uppercase tracking-widest">You're all caught up ✓</p>`;
...
function syncFeedTabBodyClasses(type) {
    const isGridTab = type === 'all' || type === 'product' || type === 'skill' || type === 'deals' || type === 'following';
...
async function _runSearchImmediate(term) {
    ...
    if (safeTerm) {
        try {
            const orFilter = ['title', 'description', 'user_name', 'institution', 'region', 'type']
            ...
            showToast("Couldn't reach the server — showing recently loaded posts only.");
        }
    }
    ...
    // here alongside it.
    if (blockedUserIds.size > 0) {
        searchResults = searchResults.filter(({ data: d }) => !blockedUserIds.has(idKey(d.user_id)));
    }
    ...
    for (const item of searchResults) {
        ...
        let score = 0;
        if (title === lower) score += 100;               // exact title match
        else if (title.startsWith(lower)) score += 60;    // title starts with query
...
window.handlePostSubmission = async function () {
    ...
    // formatting existed.
    const price       = (document.getElementById('postPrice')?.value || '').replace(/,/g, '');
    ...
    const flashSaleEnabled = !!document.getElementById('postFlashSaleToggle')?.checked;
    const originalPriceRaw = flashSaleEnabled ? (document.getElementById('postOriginalPrice')?.value.trim().replace(/,/g, '') || '') : '';
    const saleEndsRaw      = flashSaleEnabled ? (document.getElementById('postSaleEndsAt')?.value || '') : '';
    const parsedOriginalPrice = originalPriceRaw ? parseFloat(originalPriceRaw) : null;
    if (flashSaleEnabled && originalPriceRaw && (isNaN(parsedOriginalPrice) || parsedOriginalPrice < 0)) {
    ...
    if (!mediaFiles || mediaFiles.length === 0) missingRequired.push('at least one photo or video');
    ...
    if (title.length > 100) { showToast('Title must be 100 characters or fewer.'); return; }
    if (description && description.length > 2000) { showToast('Description must be 2000 characters or fewer.'); return; }

    const parsedPrice = parseFloat(price);
    if (price && (isNaN(parsedPrice) || parsedPrice < 0)) { showToast('Price can\'t be negative.'); return; }
    if (parsedPrice > 1000000) { showToast('Price seems too high — please double-check it.'); return; }
    ...
    try {
        ...
        for (let i = 0; i < mediaFiles.length; i++) {
            ...
            if (isImage) {
                try {
                    file = await compressImageFile(file, { maxDimension: 1600, quality: 0.8 });
                } catch (compressErr) {
            ...
            if (i === 0 && isImage) {
                try {
                    const thumbFile = await compressImageFile(file, { maxDimension: 400, quality: 0.7 });
                    ...
                    if (!thumbErr) {
                        const { data: { publicUrl: thumbPublicUrl } } = supabase.storage.from("posts").getPublicUrl(thumbPath);
                        thumbnailUrl = thumbPublicUrl;
                    } else {
                        console.warn('Thumbnail upload failed (non-fatal, grids fall back to full image):', thumbErr);
                    }
                } catch (thumbGenErr) {
                    console.warn('Thumbnail generation failed (non-fatal, grids fall back to full image):', thumbGenErr);
                }
            }
...
async function loadProfileStats() {
    ...
    try {
        const [followersRes, followingRes, postsRes, bioRes] = await Promise.all([
            ...
            // cosmetic.
            supabase.from("posts").select("id, title, media_url, thumbnail_url, media_type, price").eq("user_id", currentUserData.id).eq("is_archived", false).order("created_at", { ascending: false }),
            supabase.from("profiles").select("bio").eq("id", currentUserData.id).maybeSingle()
        ]);
        ...
        if (grid) {
            grid.innerHTML = postsRes.data?.map(d => renderGridItem(d.id, d)).join('') || '';
...
function saveAppSettings(partial) {
    ...
    try { localStorage.setItem(APP_SETTINGS_KEY, JSON.stringify(merged)); } catch (_) {}
    return merged;
}
...
async function loadArchivedPostsIntoSheet() {
    ...
    if (!bodyEl || !document.getElementById('info-sheet-overlay')?.classList.contains('sheet-open')) return;
    ...
    bodyEl.innerHTML = posts.map(d => {
        const thumb = pickThumbnailUrl(d);
        const archivedDate = d.archived_at ? new Date(d.archived_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : '';
        return `
...
window.openPublicProfile = async function (userId) {
    ...
    try {
        const [followersRes, followingRes, postsRes, isFollowingRes, profileRowRes] = await Promise.all([
            supabase.from('follows').select('', { count: 'exact', head: true }).eq('following_id', userId),
            supabase.from('follows').select('', { count: 'exact', head: true }).eq('follower_id', userId),
            supabase.from('posts').select('id, title, media_url, thumbnail_url, media_type, price').eq('user_id', userId).eq('is_archived', false).order('created_at', { ascending: false }),
            currentUserData
...
window.openFollowListModal = async function (userId, mode) {
    ...
    try {
        const query = mode === 'followers'
            ? supabase.from('follows').select('follower_id, follower_name, follower_avatar').eq('following_id', userId)
            : supabase.from('follows').select('following_id, following_name, following_avatar').eq('follower_id', userId);
        ...
        const rows = await Promise.all(data.map(async (row) => {
            const personId = mode === 'followers' ? row.follower_id : row.following_id;
            const name = (mode === 'followers' ? row.follower_name : row.following_name) || 'Student';
            const avatar = (mode === 'followers' ? row.follower_avatar : row.following_avatar)
            ...
            const unfollowBtn = (mode === 'following' && isOwnList) ? `
                <button
...
window.closeFollowListModal = function (fromPop = false) {
    document.getElementById('follow-list-overlay')?.classList.remove('sheet-open');
...
window.openProfilePostViewer = async function (userId, startPostId) {
    ...
    try {
        ...
        feed.innerHTML = posts.map(d => renderReelCard(idKey(d.id), d, true)).join('');
        ...
        const jumpToRequestedCard = () => {
            const targetCard = feed.querySelector(`#reel-card-${CSS.escape(idKey(startPostId))}`);
            if (!targetCard) return;
            feed.scrollTo({ top: targetCard.offsetTop, behavior: 'auto' });
            const targetMedia = targetCard.querySelector('.reel-video');
            if (targetMedia && targetMedia.dataset.src && !targetMedia.getAttribute('src')) {
...
window.closeProfilePostViewer = function (fromPop = false) {
    document.getElementById('profile-post-viewer')?.classList.remove('sheet-open', 'is-service-viewer');
    pauseAllReelVideos();
...
function renderInboxList() {
    ...
    content.innerHTML = `${inboxHeader}<div class="divide-y divide-slate-800/60 bg-slate-900 border border-slate-800/60 rounded-3xl overflow-hidden">${
...
window.openDMUserSearch = function () {
    ...
    if (!overlay) {
        ...
        overlay.className = 'hidden fixed inset-0 z-[92] bg-black/80 backdrop-blur-sm p-4';
        overlay.innerHTML = `
    ...
    setTimeout(() => document.getElementById('dm-user-search-input')?.focus(), 20);
};
...
window._runDMUserSearch = async function (term) {
    ...
    try {
        const { data, error } = await supabase
            ...
            .or(`name.ilike.%${cleanTerm}%,institution.ilike.%${cleanTerm}%,region.ilike.%${cleanTerm}%`)
            .limit(20);
        if (error) throw error;
        ...
        resultsEl.innerHTML = rows.map((row) => {
            ...
            const avatar = row.avatar || `https://ui-avatars.com/api/?background=1e293b&color=fbbf24&bold=true&name=${encodeURIComponent(displayName)}`;
            const location = [row.institution, row.region].filter(Boolean).join(' · ');
            return `
...
if (activeAuthChange) {
    activeAuthChange(async (user) => {
        ...
        if (user) {
            ...
            const authProfileNavDesktop = document.getElementById('auth-profile-nav-desktop');
            if (authProfileNavDesktop) {
                authProfileNavDesktop.innerHTML = `<i class="fas fa-user"></i><span>Profile</span>`;
                authProfileNavDesktop.onclick = function (e) { e.stopPropagation(); window.navigateTo('profile', authProfileNavDesktop); };
            }
            ...
            const authProfileNavDesktopOut = document.getElementById('auth-profile-nav-desktop');
            if (authProfileNavDesktopOut) {
                authProfileNavDesktopOut.innerHTML = `<i class="fas fa-sign-in-alt"></i><span>Sign In</span>`;
                authProfileNavDesktopOut.onclick = function (e) { e.stopPropagation(); window.openLoginModal(); };
            }
            ...
            const recentlyOnline = (Date.now() - (window.__lastSeenOnline || 0)) < 15000;
            if (typeof window.openLoginModal === 'function' && navigator.onLine && hadInitialSession && recentlyOnline) {
                window.openLoginModal();
            }
        }
...
function showGlobalErrorToast(context, err) {
    ...
    if (typeof Sentry !== 'undefined') {
        Sentry.captureException(err instanceof Error ? err : new Error(`${context}: ${err}`));
    }
...
function wireGridTileTouchHandlers() {
    document.querySelectorAll('.grid-tile[data-post-id]').forEach(tile => {
        ...
        tile.addEventListener('touchstart', () => window._startGridTileHold(key), { passive: true });
        tile.addEventListener('touchmove', () => window._cancelGridTileHold(), { passive: true });
    });
}
